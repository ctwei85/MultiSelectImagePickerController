...
{
    NSString*_toolbarDisplayText;//发送 还是完成
}
...
-(instancetype)initWithFrame:(CGRect)frame withToolbarDisplayText:(NSString*)tmpText
{
    self = [super initWithFrame:frame];
    if (self) {
        _toolbarDisplayText = tmpText;
        [self initPickerButtomView];
    }
    return self;
}
...
-(LSYSendButton *)sendButton
{
    if (!_sendButton) {
        ...
        if(_toolbarDisplayText == nil){
            _toolbarDisplayText = @"发送";
        }
        [_sendButton setTitle:_toolbarDisplayText/*@"发送"*/ forState:UIControlStateNormal];
...
-(void)initSendButton
{
    ...
    [self setTitleColor:[UIColor colorWithRed:48/255.0 green:181/255.0 blue:242/255.0 alpha:1] forState:UIControlStateNormal];
...
-(UIView *)numbersView
{
    if (!_numbersView) {
        ...
        [_numbersView setBackgroundColor:[UIColor colorWithRed:45/255.0 green:181/255.0 blue:242/255.0 alpha:1]];
